...
// Считаем что переменные названы цифрами от 0 до N, где N = <кол-во переменных> - 1. (в примере N = 2).
...
// Набор предикат с переменными.
// Класс хранит набор предикат и переменных, для удобной работы с предикатами.
// Разные предикаты могут содержать разное количество переменных, т.е. может быть P(x) и Q(x, y).
...
// 
// Переменные не могут меняться после задания хотябы одного предиката, т.к.
...
public:
   ...
   // Затем идет таблица истинности: Через пробел/запятую идут переменные до символа '=' после значение 0 или 1.
   ...
   // !> exception если нет количества аргументов, неполная таблица истинности или недостаточное кол-во аргументов в строке таблицы.
   // !> exception если отсутствует значение для строки таблицы истинности.
   // !> exception если произошла попытка переопределить значение для одинаковых аргументов.
